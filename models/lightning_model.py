...
class TextSegmenter(pl.LightningModule):
    def __init__(self, tagset_size, embedding_dim, hidden_dim, num_layers = 1,
                 ...
                 no_validation = False, all_results = False, all_scores = False):
        super().__init__()
        ...
        if architecture == 'biLSTMCRF':
          ...
          self.model = Transformer_segmenter(tagset_size, embedding_dim, hidden_dim, num_layers = num_layers, dropout_in = dropout_in, dropout_out = dropout_out, batch_first = batch_first, loss_fn = loss_fn, positional_encoding = positional_encoding, nheads = nheads, threshold = threshold)
