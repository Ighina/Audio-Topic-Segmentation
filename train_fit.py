...
def main(args):
    ...
    for fold in folds:
      ...
      test_dataset = AudioPortionDataset(fold[1], tag_to_ix, encoder = encoder, CRF =CRF, 
                                              truncate = truncate_test, umap_project = args.pca_reduce, umap_project_value = args.pca_value,
                                              umap_class = train_dataset.reducer)
    ...
    for param_index, param_tuple in enumerate(hyperparameters):
        ...
        for index, segm in enumerate(loaders):
            ...
            elif args.encoder.startswith('prosodic'):
                embedding_dim = 167          
            
            elif args.encoder.startswith('openl3_std') or args.encoder.startswith('openl3/_mean_std'):
                embedding_dim = 1024

            elif args.encoder.startswith('wav2vec_std') or args.encoder.startswith('wav2vec/_mean_std'):
                embedding_dim = 1536

            elif args.encoder.startswith('x-vector') or args.encoder.startswith('openl3') or args.encoder.startswith('crepe_std') or args.encoder.startswith('crepe/_mean_std'):
                embedding_dim = 512

            elif args.encoder.startswith('crepe'):
                embedding_dim = 256

            elif args.encoder.startswith('mfcc'):
                embedding_dim = 200

            elif args.encoder.startswith('ecapa'):
                embedding_dim = 192

            elif args.encoder.startswith('wav2vec'):
                embedding_dim = 768

            else:
                raise ValueError("Encoder not recognised, use one of the three available options (x-vectors, ecapa or wav2vec)")
